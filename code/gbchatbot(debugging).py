...
data = pd.read_csv('./100qas.csv')
questions, answers = list(data['questions']), list(data['answers'])
X_train, y_train = questions, answers
...
def train(questions, answers):
  ...
  while(epoch<20):
    ...
    g_total = 0
    iter = 0
    for start, end in zip(range(0, len(questions) - batch_size, batch_size), range(batch_size, len(questions), batch_size)):
      batch_questions_labeled = tokenizer(questions[start:end-num_unlabeled], padding='max_length', truncation=True, return_tensors='pt', max_length=seq_len)
      batch_questions_unlabeled = tokenizer(questions[end-num_unlabeled:end], padding='max_length', truncation=True, return_tensors='pt', max_length=seq_len)
      batch_answers = tokenizer(answers[start:end-num_unlabeled], return_tensors="pt", padding='max_length', truncation=True, max_length=seq_len)
      ...
      us_total += d_unsupervised_loss


      z = np.random.normal(0,1,(num_unlabeled,z_dim))
      generator_loss, _ = gan.train_on_batch(z,real[:num_unlabeled])
      g_total += generator_loss
      iter+=1

    print("Epoch No.:",epoch,end=",")
    print("Discriminator Supervised Loss:",s_total/iter,end=',')
    print('Generator Loss:',g_total/iter,end=",")
    print('Discriminator Unsupervised Loss:',us_total/iter,sep=',')
    epoch += 1
...
discriminator_supervised.save_weights('./discriminator_supervised_saved_weights')
...
out = ds.generate_answer([X_train[0]], 101, model, tokenizer)
print()
